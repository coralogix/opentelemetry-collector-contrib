...
func retrieveURIForProvider(uri string) (string, string) {
	...
	// Files that contain a colon in the name worked before
	// even though the configmap lib doesn't support them.
	// This tries to keep backwards compatibility by handling
	// them as file URIs if the path exists on the filesystem.
...
// It preserves the service.extensions list by appending (with deduplication)
// rather than overwriting, matching the behavior of the Collector's config merge.
...
// MergeConf merges the incoming Conf into base. It preserves the
// service.extensions list by appending (with deduplication) rather than
// overwriting.
func MergeConf(base, incoming *confmap.Conf) error {
	...
	// Restore the merged extensions list: base extensions first, then incoming,
	// deduplicated by first occurrence.
	if len(baseExtensions) > 0 || len(incomingExtensions) > 0 {
		merged := deduplicateExtensions(baseExtensions, incomingExtensions)
		setExtensions(base, merged)
	}

	return nil
}

// getExtensions extracts the service::extensions list from a Conf.
...
// setExtensions sets the service::extensions list in a Conf by creating
// a small overlay and merging it.
...
// deduplicateExtensions concatenates two extension lists and removes duplicates,
// preserving order with first occurrence winning.
