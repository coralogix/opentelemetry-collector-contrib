...
var errConfigNotRabbit = errors.New("config was not a Zabbix receiver config")
...
func createMetricsReceiver(ctx context.Context, params component.ReceiverCreateSettings, rConf config.Receiver, consumer consumer.Metrics) (component.MetricsReceiver, error) {
	...
	zabbixScraper := newScraper(params.Logger, cfg, params)
	scraper, err := scraperhelper.NewScraper(typeStr, zabbixScraper.scrape, scraperhelper.WithStart(zabbixScraper.start))
	if err != nil {
		return nil, err
	}
